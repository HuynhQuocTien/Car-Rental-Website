...
function calculateDiscountPercentage(originalPrice, discountedPrice) {
    ...
    return roundedDiscount % 1 === 0 ? roundedDiscount.toFixed(0) : roundedDiscount.toFixed(2);
}
...
Dashmix.onLoad((() => class {
    static initValidation() {
        Dashmix.helpers("jq-validation"), jQuery("#addVehicleForm").validate({
            rules: {
                "makeID": { required: !0 },
...
const renderData = function(vehicles) {
    ...
    vehicles.forEach((vehicle) => {
        html += `<tr>
            ...
            <td>
                ${vehicle.Active == 1 ? '<span class="badge bg-success">Available</span>' : '<span class="badge bg-danger">Unavailable</span>'}             </td>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    vehiclePagination.getPagination(vehiclePagination.option, vehiclePagination.valuePage.curPage);
                } else {
                    Dashmix.helpers('jq-notify', { 
                        type: 'danger', 
    ...
    $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $("#weeklyDiscount").val(calculateDiscountPercentage(vehicle.DailyPrice, vehicle.WeeklyPrice/7) || 0);
                    $("#monthlyDiscount").val(calculateDiscountPercentage(vehicle.DailyPrice, vehicle.MonthlyPrice/30) || 0);
